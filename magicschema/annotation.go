...
type Annotator interface {
	...
	// Prepare provides the full file content to annotators that need
	// file-level context (e.g., bitnami's ## @param annotations which
	// are not attached to individual AST nodes). Annotators that don't
	// need file-level context may implement this as a no-op.
	// Called once per input file, before any Annotate calls for that file.
	// Implementations must reset any per-file state from previous calls.
	Prepare(content []byte) error
