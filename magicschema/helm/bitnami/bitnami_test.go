...
func TestBitnamiPrepare(t *testing.T) {
	...
	for name, tc := range tcs {
		t.Run(name, func(t *testing.T) {
			...
			// For the "prepare resets state" tests, first prepare with old content.
			if name == "prepare resets state from previous call" {
				err := ann.Prepare([]byte(stringtest.Input(`
			...
			err := ann.Prepare([]byte(tc.content))
			require.NoError(t, err)

			result := ann.Annotate(nil, tc.keyPath)
			tc.want(t, result)
		})
	}
}
