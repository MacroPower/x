...
// Prepare scans the full file content for ## @param annotations and builds
// a key-path-to-annotation map.
func (a *Annotator) Prepare(content []byte) error {
	a.params = make(map[string]*bitnamiParam)
	a.skips = make(map[string]bool)
	...
	for line := range lines {
		...
		if m := skipRegex.FindStringSubmatch(line); m != nil {
			a.skips[normalizeKeyPath(m[1])] = true
