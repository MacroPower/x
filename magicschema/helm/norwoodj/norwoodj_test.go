...
func TestHelmDocsAnnotator(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"default with empty value": {
			input: stringtest.JoinLF("# @default -- ", "# -- Description", "val: actual"),
			want: func(t *testing.T, got map[string]any) {
				t.Helper()
		...
		"default before multiple dash-dash lines is preserved": {
			// PRD improvement: @default before the last # -- group should be preserved.
			input: stringtest.Input(`
...
func TestHelmDocsAnnotatorPrepareResetsState(t *testing.T) {
	t.Parallel()

	// First file has an old-style comment for key.path.
	// Second file has no comments.
	// After a second Prepare call, the old-style state should be reset.
	ann := norwoodj.New()
	...
	// First Prepare with old-style comment.
	require.NoError(t, ann.Prepare([]byte(firstFile)))

	// Second Prepare with no comments should reset state.
	require.NoError(t, ann.Prepare([]byte(secondFile)))
	...
	// No description should be set since Prepare was called again
