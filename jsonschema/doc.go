...
//   - A name a composed embed promotes (see Struct Fields) takes a seeded
//     default on a true placeholder property beside allOf, under every
//     draft and closing choice, so the embed's branch still judges the name
//     and the placeholder carries the default alone. Without a seeded key
//     only an object closing with unevaluatedProperties emits the
//     placeholder, since that close needs the parent to evaluate the name.
...
//     map or an anonymous struct keeps the value schema or the property
//     schemas. Each keeps its own null admission, tag, description, and
//     interpreter output, so a pointer property under a type=object override
//     still admits null. A slice, map, or provider-declared type extracted
//     to $defs keeps them too: the override applies to a copy of the
//     definition, so the occurrence renders inline with the definition's
//     element schema and keywords, as the inline type would, while every
//     other occurrence keeps the reference. A named struct is the one
//     exception; type=object on it is a leaf object, whether the struct is
//     extracted to $defs or inlined by [WithDefinitions], so the setting
//     changes nothing the override means.
...
// holding one sub-schema (items, not, contains), or a null member of
// Draft-07 dependencies, returns an error wrapping [ErrNilSubschema], since
// unmarshaling into a [Schema] directly reads it as the false schema, and a
// null element of a string list (required, a
...
// properties, dependentSchemas) and a member of dependentRequired
// included, where the metaschema types the keyword and refuses the null.
//
...
// caps a bound parsed at an integer-kind field at 2^53 outright. A scalar
// past the width of the field's own kind (const=200 on an int8) fails with
// the same sentinel, as does a bound past int64, uint64, or the float
// kind's range (1e300 on a float32). An integer bound inside 2^53 but past
// a narrower field's width is not refused: it intersects with the type's
// own bound under the intersect-only rule, and a floor above the type's
// ceiling renders as the unsatisfiable interval it is.
//
...
//     not. A name defined twice is refused where both groups can take part
...
//     which §3.2.2 admits and net/url refuses. A bracketed host is the RFC
//     3986 IPv6address or IPvFuture literal as written, so an RFC 6874 zone
//     identifier ("[fe80::1%25en0]"), which net/url accepts, is refused.
//     Every component, the query included, must spell each '%' as a
//     pct-encoded triplet, and an authority carries at most one raw '@'
//     (§3.2.1). The iri forms follow the same rules.
