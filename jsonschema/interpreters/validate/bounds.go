...
func applyMinBound(field **int, effMin *int, value string, exclusive bool) error {
	...
	raiseFloor(field, effMin, clampNonNegative(inclusiveLowerBound(n, exclusive)))
...
// would instead accept the empty value the rule forbids (go-playground's lt=0
// rejects every value, including the empty one), so the contradiction is
// expressed as a floor of one against a ceiling of zero, mirroring how an
// incompatible len yields an unsatisfiable min/max range.
func applyMaxBound(minField, maxField **int, effMin, effMax *int, value string, exclusive bool) error {
	n, err := parseBoundValue(value)
	if err != nil {
		return err
	}
	...
	if ceiling < 0 {
		raiseFloor(minField, effMin, 1)
		lowerCeiling(maxField, effMax, 0)

		return nil
	}

	lowerCeiling(maxField, effMax, ceiling)
...
// satisfied -- no string or collection has a negative length, and go-playground
// rejects every value for len=-1 -- so it is expressed as the unsatisfiable
// floor-one/ceiling-zero range (as applyMaxBound does for a sub-zero ceiling),
// rather than clamping to 0, which would accept the empty value the rule forbids.
func applyLenBound(minField, maxField **int, effMin, effMax *int, value string) error {
	n, err := parseBoundValue(value)
	if err != nil {
		return err
	}
	...
	raiseFloor(minField, effMin, n)
	lowerCeiling(maxField, effMax, n)
