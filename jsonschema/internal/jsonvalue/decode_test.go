...
func TestDecode(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got, err := decodeAny([]byte(tt.input))
			if tt.err != "" {
...
func TestDecodeSyntacticError(t *testing.T) {
	t.Parallel()
...
// TestDecodeReusesDecoder pins the pooled decoder: a document
...
func TestDecodeReusesDecoder(t *testing.T) {
	t.Parallel()

	_, err := decodeAny([]byte(`{"a": [1, `))
	require.Error(t, err)

	first := []byte(`{"key": "value", "n": 12345}`)

	got, err := decodeAny(first)
	require.NoError(t, err)
	...
	_, err = decodeAny([]byte(`["other", 99999]`))
	require.NoError(t, err)
...
func BenchmarkDecodeReader(b *testing.B) {
	...
	for b.Loop() {
		_, err := decodeReaderAny(strings.NewReader(data))
		if err != nil {
			b.Fatal(err)
		}
	}
...
// decodeAny decodes data and returns the legacy any form, so the table
// expectations read as JSON-shaped Go values.
...
// decodeReaderAny is decodeAny over an io.Reader.
func decodeReaderAny(r io.Reader) (any, error) {
	v, err := jsonvalue.DecodeReader(r)
	if err != nil {
		return nil, err
	}
